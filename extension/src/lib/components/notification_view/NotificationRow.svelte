<script lang="ts">
	...
	// Ported from frontend/src/lib/components/notification_view/NotificationRow.svelte.
	...
	//  4. No keyboard focus index — the panel has no list-level keyboard navigation,
	//     so rows are reachable by tab order alone.
	...
	import { PANEL_CONTROLLER_KEY, type NotificationPageController } from "$lib/state/types";
	...
	const pageController = getContext<NotificationPageController>(PANEL_CONTROLLER_KEY);
	const { snoozeDropdownState, tagDropdownState } = pageController.stores;
	...
	const actionButtonClass =
		"flex h-9 w-9 items-center justify-center rounded-full bg-transparent text-gray-700 dark:text-gray-300 transition hover:bg-gray-200 dark:hover:bg-gray-700/50 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-600/40 cursor-pointer";

	// The desktop row computes these two per read/unread, but both branches are the
	// same string there, so they are plain constants here.
	const buttonGroupBg = "bg-gray-50 dark:bg-gray-950";
	const borderClass = "border-gray-300 dark:border-gray-800";

	// Subscribing to the time store keeps same-day relative timestamps fresh.
	...
	$: notificationKey = notification.githubId ?? notification.id;
	$: isSnoozeDropdownOpen = $snoozeDropdownState?.notificationId === notificationKey;
	$: isTagDropdownOpen = $tagDropdownState?.notificationId === notificationKey;
	...
	const openSubject = () => {
		void pageController.actions.openInGitHub(notification);
	};
	...
	function handleSnoozeClick() {
		if (isSnoozeDropdownOpen) {
			...
			pageController.actions.openSnoozeDropdown(notificationKey, "notification-row");
		}
	}
	...
	function handleTagClick() {
		if (isTagDropdownOpen) {
			...
			pageController.actions.openTagDropdown(notificationKey, "notification-row");
		}
	}
...
<div class="group/row relative flex items-start gap-1">
	<!-- Vertical Status Bar -->
	<div class="flex w-5 shrink-0 items-start justify-center self-stretch pt-2.5">
		<div class="ml-auto mr-1 h-10 w-1 rounded-full {statusBarColor}" aria-hidden="true"></div>
	</div>

	<div
		class={`group relative flex flex-1 cursor-pointer gap-2.5 rounded-lg border px-2.5 py-2 transition ${baseBackground} ${hoverBackground} ${borderClass} focus-visible:outline-none focus-visible:border-blue-600`}
		data-notification-key={notificationKey}
		role="button"
		tabindex="0"
		aria-label={`Open ${notification.subjectTitle} on GitHub`}
	...
	>
		<div class="flex-1 space-y-1.5 text-left">
			...
			<div class="flex items-start gap-2">
				<div class="mt-0.5 flex-shrink-0">
					<svg
						class="h-4 w-4 {iconConfig.colorClass} {iconOpacity}"
						viewBox="0 0 16 16"
						fill="currentColor"
						aria-hidden="true"
					>
				...
				<div class="flex flex-1 flex-wrap items-center gap-1.5">
					{#if isMuted}
						<span
							class="rounded-md bg-violet-500/10 px-1.5 py-0.5 text-[10px] font-medium text-violet-600 dark:bg-violet-500/20 dark:text-violet-300"
						>
							Muted
						</span>
					{:else if isArchived}
						<span
							class="rounded-md bg-blue-500/10 px-1.5 py-0.5 text-[10px] font-medium text-blue-600 dark:bg-blue-500/20 dark:text-blue-300"
						>
							Archived
						</span>
					{:else if notification.filtered}
						<span
							class="rounded-md bg-gray-500/10 px-1.5 py-0.5 text-[10px] font-medium text-gray-700 dark:bg-gray-500/20 dark:text-gray-300"
						>
							Skipped Inbox
						</span>
					{/if}
			...
			<div class="flex flex-wrap items-center gap-1.5 pl-6">
				<span
					class={`text-sm ${isUnread ? "font-medium text-gray-600 dark:font-normal" : "font-normal text-gray-500"} dark:text-gray-400`}
					>{repoLabel}</span
				>
		...
		<div class="flex flex-col items-end justify-start self-stretch">
			...
			{#if snoozeMessage || notification.starred || isMuted}
				...
				>
					{#if snoozeMessage}
						<span
							class="whitespace-nowrap text-xs font-medium text-orange-600 dark:text-orange-400"
		...
		>
			<div class="{buttonGroupBg} flex items-center gap-0.5 rounded-full p-0.5 shadow-lg">
				{#if subjectLink}
					<a
				...
				<div class="relative">
					...
					<TagDropdown
						...
						onAssignTag={(tagId) => pageController.actions.assignTag(notificationKey, tagId)}
						onRemoveTag={(tagId) => pageController.actions.removeTag(notificationKey, tagId)}
						onClose={pageController.actions.closeTagDropdown}
					/>
				</div>

				<button
					type="button"
					class="{actionButtonClass} {notification.starred ? 'text-violet-500' : ''}"
					on:click|stopPropagation={() =>
				...
				<div class="relative">
					...
					<SnoozeDropdown
						...
						onSnooze={(until) => void pageController.actions.snooze(notification, until)}
						onUnsnooze={() => void pageController.actions.unsnooze(notification)}
						onOpenCustomDialog={() =>
							pageController.actions.openCustomSnoozeDialog(notificationKey)}
						onClose={pageController.actions.closeSnoozeDropdown}
					/>
				</div>
			</div>
		</footer>
