<script lang="ts">
	...
	// Adopted from frontend/src/lib/components/timeline/ViewDropdown.svelte, which
	// is dead code there — nothing in the desktop app imports it. The panel is now
	// its only consumer, so it is owned here rather than vendored, and two stale
	// bits are fixed: `starred` had no icon (rendering an empty, misaligned span
	// for a view the backend does return) and `done` mapped an icon for a built-in
