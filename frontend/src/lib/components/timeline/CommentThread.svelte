<script lang="ts">
	...
	import type { CommentsController } from "$lib/state/commentsController";
	import Comment from "./Comment.svelte";
	import { writable } from "svelte/store";
	import { tick } from "svelte";

	export let githubId: string;
	export let commentsController: CommentsController;
	export let totalComments: number = 0; // Total comment count from API

	const { items, isLoading, error, pagination } = commentsController.stores;
	const { loadComments, loadMoreComments } = commentsController.actions;
	...
	async function handleLoadComments() {
		...
		try {
			await loadComments(githubId, 10);
			await tick();
		} finally {
	...
	async function handleLoadMore() {
		isLoadingMore.set(true);
		try {
			await loadMoreComments(githubId, 10);
			await tick();
		} finally {
			isLoadingMore.set(false);
		}
	...
	// Don't switch to hasItems view until loading is complete
	$: showInitialButton = !hasLoaded || ($isLoadingMore && !hasItems);
	// Show no comments state if total is 0
	$: showNoComments = totalComments === 0 && !hasLoaded && !$isLoadingMore;
	// Format the button text
	$: loadCommentsText =
		totalComments > 0
			? `Load ${totalComments} comment${totalComments === 1 ? "" : "s"}`
			: "Load comments";
</script>

{#if showNoComments}
	<!-- No comments state (shown when total is 0) -->
	<div class="relative">
		<div class="flex gap-3 pt-4">
			...
			<div
				class="flex-1 rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-100/30 dark:bg-gray-800/30 px-4 py-3"
			>
	...
	<div class="relative">
		<div class="flex gap-3 pt-4">
			...
			<div class="flex-1">
				<button
					type="button"
					class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-100/50 dark:bg-gray-800/50 px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-200 transition hover:bg-gray-200 dark:hover:bg-gray-800 hover:border-gray-400 dark:hover:border-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600/40 disabled:opacity-50 disabled:cursor-not-allowed"
					on:click={handleLoadComments}
		...
		<div class="flex gap-3 pt-4">
			...
			<div class="flex-1 rounded-lg border border-red-800 bg-red-900/20 px-4 py-3">
				<div class="flex items-start gap-3">
					...
					<div class="flex-1">
						<p class="text-sm font-medium text-red-200">Failed to load comments</p>
...
{:else if hasItems}
	<!-- Comment thread with continuous line -->
	<div class="relative">
		<!-- Load more button (at top, for older comments) -->
		...
		{#each $items as item, index (item.id)}
			<Comment {item} showThread={false} isLastItem={index === lastItemIndex} />
		{/each}
	</div>
{:else if !$isLoading}
	<!-- No comments state -->
