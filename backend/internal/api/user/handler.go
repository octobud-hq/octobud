...
func (h *Handler) HandleSetGitHubToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if strings.Contains(errMsg, "unauthorized") {
			shared.WriteError(w, http.StatusBadRequest, "Invalid token: authentication failed")
			return
		}
		...
		shared.WriteError(w, http.StatusBadRequest, "Failed to validate token: "+errMsg)
		return
	}
...
func (h *Handler) HandleClearGitHubToken(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.tokenManager.ClearToken(ctx); err != nil {
		...
		shared.WriteError(w, http.StatusInternalServerError, "Failed to clear token")
		return
	}
...
func (h *Handler) HandleDeleteAllGitHubData(w http.ResponseWriter, r *http.Request) {
	if h.store == nil {
		shared.WriteError(w, http.StatusInternalServerError, "Database store not configured")
		return
	}
	...
	if err != nil {
		...
		shared.WriteError(w, http.StatusInternalServerError, "Failed to delete GitHub data")
		return
	}
...
func (h *Handler) HandleSetMute(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		shared.WriteError(w, http.StatusInternalServerError, "Failed to update mute status")
		return
	}
...
func (h *Handler) HandleClearMute(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		shared.WriteError(w, http.StatusInternalServerError, "Failed to clear mute status")
		return
	}
